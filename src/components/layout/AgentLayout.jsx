        ...
        <div className="flex items-center justify-around px-2 py-2">
          {[
            ...
            <NavLink key={item.to} to={item.to}
              className={({ isActive }) =>
                item.fab
                  ? `flex flex-col items-center gap-1 px-3 py-1 transition-colors ${isActive ? 'text-primary' : 'text-slate-400'}`
                  : `flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-colors ${isActive ? 'text-primary' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`
              }
            >
              {({ isActive }) => item.fab ? (
                <>
