...
export default function AgentDashboard() {
  ...
  const authorises = myLogs.filter(l => l.resultat === 'autorisé').length
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-100 dark:bg-bg-dark p-4 space-y-4">
      ...
      <div className="bg-primary rounded-2xl p-5 text-white shadow-lg shadow-primary/20">
        ...
        <p className="text-blue-200 text-sm mt-0.5">{myPC?.nom || 'Point de contrôle non assigné'}</p>
        <div className="flex items-center gap-2 mt-3">
          <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></div>
          <span className="text-xs text-emerald-300 font-medium">Terminal actif · Cache synchronisé</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Scans", value: myPC?.scans ?? myLogs.length, color: "text-slate-900 dark:text-white" },
          { label: "Autorisés", value: authorises, color: "text-emerald-600 dark:text-emerald-400" },
          { label: "Alertes",   value: alertes,    color: alertes > 0 ? "text-red-500" : "text-slate-400 dark:text-slate-500" },
        ].map(s => (
      ...
      <button onClick={() => navigate('/agent/scanner')}
        ...
        <div className="text-center">
          <p className="text-lg font-bold">Scanner un badge</p>
          <p className="text-blue-200 text-sm">Appuyez pour activer la caméra</p>
        </div>
      </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { icon: 'wifi',         bg: 'bg-emerald-100 dark:bg-emerald-900/30', ic: 'text-emerald-600 dark:text-emerald-400', title: 'Réseau',         val: 'En ligne',        valc: 'text-emerald-600 dark:text-emerald-400' },
          { icon: 'sync',         bg: 'bg-blue-100 dark:bg-blue-900/30',      ic: 'text-blue-600 dark:text-blue-400',      title: 'Cache',          val: 'Sync il y a 2min', valc: 'text-blue-600 dark:text-blue-400' },
          { icon: 'map',          bg: 'bg-purple-100 dark:bg-purple-900/30',  ic: 'text-purple-600 dark:text-purple-400',  title: 'Zones actives',  val: 'Z1 · Z2 · Z3',    valc: 'text-purple-600 dark:text-purple-400' },
          { icon: 'offline_bolt', bg: 'bg-amber-100 dark:bg-amber-900/30',    ic: 'text-amber-600 dark:text-amber-400',    title: 'Mode hors ligne',val: '4h disponibles',   valc: 'text-amber-600 dark:text-amber-400' },
        ].map(s => (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-slate-800">
          <h3 className="text-sm font-bold text-slate-800 dark:text-white">Scans récents</h3>
          <button onClick={() => navigate('/agent/history')} className="text-xs text-primary font-semibold hover:underline">Voir tout</button>
        </div>
        <ul className="divide-y divide-slate-50 dark:divide-slate-800">
          {myLogs.length === 0 && (
            <li className="px-5 py-8 text-center text-sm text-slate-400 dark:text-slate-500">Aucun scan pour cette session</li>
          )}
          {myLogs.map(log => {
